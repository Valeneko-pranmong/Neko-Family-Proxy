...
[ PHASE 2: REMOTE CI AUTHORITY GATE (GitHub Actions Hosted Runner) ]
 ...
 13. Run verify_github_release_assets.py:
     ...
     - Fail-closed first-release value checks (sequence=1, min_seq=1, string release_id=stable-0001,
       protocol=1..1, component versions=5.1.0a3)
...
4. **Construct and Sign Canonical Manifest (release-v2.json)**:
   ...
   - `channel`: `stable`
   - `release_sequence`: `1` (for this initial release; strictly monotonic positive integer)
   - `minimum_supported_sequence`: `1`
   - `release_id`: `stable-0001` (signed string identifier; see Section 4.5 for distinction from GitHub numeric release ID)
...
1. **Signed Manifest Envelope Release ID** (`manifest_doc['release_id']`):
   A string value embedded inside the cryptographically signed `release-v2.json` envelope that uniquely designates the logical release identity within the application update protocol (for this initial release, strictly `"stable-0001"`).
...
5. **First-Release Fail-Closed Parameters (Normative for v5.1.0a3)**:
   ...
   - `release_sequence` is strictly `1`.
   - `minimum_supported_sequence` is strictly `1`.
   - Signed string `release_id` is strictly `stable-0001` (clearly distinguished from GitHub's numeric release ID).
...
6. **First-Release Parameter Invariance**: For this initial release `v5.1.0a3`, any discrepancy in channel (`stable`), release sequence (`1`), minimum supported sequence (`1`), signed envelope release ID (`stable-0001`), updater protocol (`1..1`), or component versions (`5.1.0a3`) causes immediate publication abortion.
...
| Signed Envelope Release Identifier | `stable-0001` | Unique release envelope identity string in manifest |
...
- **Mitigation**: The client updater enforces durable local high-water marks and rejects any release where `release_sequence <= current_sequence`. The workflow also validates that `release_sequence` matches the expected sequence (`1` for the first release).
...
- [ ] **Gate #2 Step 1 (Freeze)**: Isolate candidate files in dedicated local candidate staging directory; lock candidate files against any further compilation or editing.
- [ ] **Gate #2 Step 2 (Smoke)**: Execute Launcher packaged smoke tests and Updater self-check directly on those exact frozen bytes.
- [ ] **Gate #2 Step 3 (Fresh Measurement)**: Freshly recompute Launcher/Updater/Core sizes+SHA-256 digests and Core installed identity SHA-256 strictly after smoke completion.
- [ ] **Gate #2 Step 4 (Sign)**: Construct and sign canonical `release-v2.json` using fresh measurements and offline Ed25519 private key (`key_id: neko-update-prod-1`, sequence `1`, minimum sequence `1`, string `release_id: stable-0001`, protocol range `1..1`, component versions `5.1.0a3`).
...
- [ ] **Gate #2 Step 7 (Review Approval)**: Obtain formal independent review approval with Critical 0 and Important 0 (C0/I0) findings.
...
- [ ] Enforce first-release fail-closed parameter validation: `channel=stable`, `release_sequence=1`, `minimum_supported_sequence=1`, signed string `release_id=stable-0001`, `updater_protocol` minimum=1 maximum=1, component versions `launcher/updater/core=5.1.0a3`, and key/tag binding remains `v5.1.0a3`.
...
5. **First-Release Fail-Closed Invariants**: For this first public release `v5.1.0a3`, Phase 2 publication authority MUST reject unless the signed envelope has `channel=stable`, `release_sequence=1`, `minimum_supported_sequence=1`, signed string `release_id=stable-0001`, `updater_protocol` minimum=1 maximum=1, launcher/updater/core versions all `5.1.0a3`, and key/tag binding remains `v5.1.0a3`. Future releases may generalize expected values only through reviewed release input/authority rules.
