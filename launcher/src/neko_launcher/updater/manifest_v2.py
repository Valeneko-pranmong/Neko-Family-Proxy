...
UPDATER_PROTOCOL_VERSION: int = 1
...
def parse_release_v2(payload: dict[str, Any]) -> ReleaseSetV2:
    ...
    if not isinstance(comps, dict) or set(comps.keys()) != {"launcher", "updater", "core"}:
        raise ValueError("components must contain exactly 'launcher', 'updater', and 'core'")
    ...
    for name in ("launcher", "updater", "core"):
        ...
        if name == "launcher":
            ...
            if fmt != "raw-pe-v1":
                raise ValueError("Updater artifact_format must be 'raw-pe-v1'")
            if size > 134217728:  # 128 MiB
                raise ValueError("Updater artifact exceeds 128 MiB")
            if art_sha != inst_sha:
...
def parse_legacy_recovery_release_v2(payload: dict[str, Any]) -> ReleaseSetV2:
    if not isinstance(payload, dict):
        raise ValueError("Payload must be a dict")
    if set(payload.keys()) != _V2_TOP_KEYS:
        raise ValueError("Invalid top-level fields for ReleaseSetV2")
...
def verify_legacy_recovery_envelope_v2(
    document: object,
    key_registry: Mapping[str, bytes],
) -> tuple[ReleaseSetV2, str]:
    """Verify an Ed25519-signed legacy release envelope for stored recovery only."""
    if not isinstance(document, dict):
        raise ValueError("Envelope must be a dict")
    if set(document.keys()) != _ENVELOPE_KEYS:
        raise ValueError("Invalid envelope schema keys")
    ...
    release_set = parse_legacy_recovery_release_v2(payload_obj)
    return release_set, payload_sha256
