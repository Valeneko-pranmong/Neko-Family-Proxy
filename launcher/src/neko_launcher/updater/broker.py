...
class BrokerCoordinator:
    ...
    def _default_verifier(
        self, published_path: Path, candidate: Generation, envelope_bytes: bytes
    ) -> None:
        ...
        if (
            release_set.release_sequence != candidate.binding.release_sequence
            or release_set.release_id != candidate.binding.release_id
        ):
            raise ValueError("PACKAGE_INVALID")

        if (
            release_set.components["launcher"].installed_identity_sha256
        ...
        if (
            ...
            != candidate.core_identity_sha256
        ):
            raise ValueError("PACKAGE_INVALID")
    ...
    def admit_authority(self, envelope_b64: str) -> AuthorityAdmissionResponse:
        ...
        result, next_state = handle_authority_admission_request(
            selection.state, envelope_b64, self.public_keys
        )
    ...
    def begin(
        ...
        result, next_state = handle_begin_request(
            self.root_dir,
            selection.state,
            envelope_b64,
            self.public_keys,
    ...
    def apply(self, transaction_id: str, request_id: str) -> ApplyResult:
        ...
        if (
            ...
            or selection.state is None
            or selection.state.phase != "PREPARING"
            or selection.state.transaction is None
            or selection.state.transaction.id != transaction_id
            or selection.state.transaction.request_id != request_id
        ):
        ...
        if tx.repair_components is not None:
            if "launcher" in tx.repair_components:
                expected_files.add("launcher.artifact")
            if "core" in tx.repair_components:
                expected_files.add("core.artifact.zip")
        else:
            if (
                tx.old is None
                or tx.old.launcher_identity_sha256 != tx.candidate.launcher_identity_sha256
            ):
                expected_files.add("launcher.artifact")
        ...
        staging_handoff_res = handle_apply_request(
            self.root_dir, current_state, transaction_id, request_id, self.public_keys
        )
        ...
        new_tx = dataclasses.replace(
            ...
            staging=identity,
            stage="BUILDING",
            mutation=Mutation("WRITE_CANDIDATE", "stage", "INTENT"),
        )
        building_state = dataclasses.replace(
            current_state, transaction=new_tx, revision=current_state.revision + 1
        )
        try:
        ...
        try:
            ...
            if (
                building_state.committed is not None
                and building_state.highwater == tx.candidate.binding
            ):
                build_state = dataclasses.replace(
                    building_state, highwater=building_state.committed.binding
                )
            ...
            is_repair = tx.repair_components is not None
            published_path = publisher.publish_generation(
                build_result.generation_dir,
                build_result.generation_id,
                repair=is_repair,
            )
        ...
        verified_state = dataclasses.replace(
            building_state, transaction=verified_tx, revision=building_state.revision + 1
        )
        try:
