...
def test_stable_version_is_declared_only_on_canonical_surfaces() -> None:
    ...
    patterns = {
        launcher_root / "pyproject.toml": re.compile(r'^version\s*=\s*"5\.1\.5"$', re.MULTILINE),
        launcher_root / "src" / "neko_launcher" / "__init__.py": re.compile(
            r'^__version__\s*=\s*"5\.1\.5"$', re.MULTILINE
        ),
    }
    ...
    assert declarations == ["pyproject.toml", "src/neko_launcher/__init__.py"], (
        "production must declare stable 5.1.5 only in pyproject.toml and neko_launcher.__version__"
    )
