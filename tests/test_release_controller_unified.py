...
def test_legacy_publish_split_release_rejects_setup_in_machine_dir(tmp_path):
    sha = "a" * 40
    tag = "v5.1.1"
...
def test_legacy_publish_split_release_rejects_missing_or_extra_installer(monkeypatch, tmp_path):
    sha = "a" * 40
    tag = "v5.1.1"
    machine_dir = tmp_path / "machine"
    machine_dir.mkdir()
...
def test_legacy_publish_split_release_order_and_cutover(monkeypatch, tmp_path):
    tag = "v5.1.1"
    sha = "3" * 40
    installer_repo = "Valeneko-pranmong/Neko-Family-Proxy-Installer"
...
def test_legacy_publish_split_release_installer_failure_stops_machine_cutover(monkeypatch, tmp_path):
    tag = "v5.1.1"
    sha = "4" * 40
    installer_repo = "Valeneko-pranmong/Neko-Family-Proxy-Installer"
...
def test_legacy_publish_split_release_installer_verify_failure_stops_cutover(monkeypatch, tmp_path):
    tag = "v5.1.1"
    sha = "5" * 40
    installer_repo = "Valeneko-pranmong/Neko-Family-Proxy-Installer"
...
def test_process_accepted_commits_unified_build_and_provenance(monkeypatch, tmp_path):
    sha = "7" * 40
    run_id = 99999
    installer_repo = "Valeneko-pranmong/Neko-Family-Proxy-Installer"
    ...
    def fake_run(args, **kwargs):
        ...
        if "build_software_release_v2.py" in str(args[1]):
            ...
            in_idx = args.index("--input")
            meta = json.loads(Path(args[in_idx + 1]).read_bytes())
            meta["source_commit"] = "7777777777777777777777777777777777777777"
            meta["component_set_sha256"] = "c" * 64
            payload_str = json.dumps(meta)
            fake_env = {
                "envelope_version": 1,
                "key_id": "neko-update-prod-1",
                "payload_b64": base64.b64encode(payload_str.encode("utf-8")).decode("ascii"),
                "payload_sha256": "p" * 64,
                "signature": {"key_id": "neko-update-prod-1"},
                "signature_b64": base64.b64encode(b"s" * 64).decode("ascii"),
            }
            from neko_launcher.updater.canonical_json import canonical_json_dumps
            Path(args[out_idx + 1]).write_bytes(canonical_json_dumps(fake_env) + bytes([10]))
            return subprocess.CompletedProcess(args, 0, stdout="", stderr="")
        return subprocess.CompletedProcess(args, 0, stdout="", stderr="")
    ...
    unified_publish_calls = []
    monkeypatch.setattr(
        "scripts.publish_atomic_release.publish_unified_release",
        lambda **kwargs: unified_publish_calls.append(kwargs),
    )

    process_accepted_commits(sha, run_id, installer_repo=installer_repo)

    assert len(unified_publish_calls) == 1
    call = unified_publish_calls[0]
    assert call["tag"] == "v5.1.2"
    assert call["target_commit"] == sha


    m_dir = Path(call["staging_dir"])
